...
###  Библиотека примеров - HC32L110   VSCode + EIDE 
...
```
  ...
  - дебагер по умолчанию JLink 
    ( проверено с JLink OB v2 -  прошивается и отлаживается )
  - настроен pyocd 
...
```
  тестирование spi
  поддержка только 8 бит

  отправка двух байт  в цикле
  после отправки - ожидаем  SPIF  и обязательно читаем DATA 
  только после отправляем следующий байт или завершаем сигналом  CS.
  сигналы NSS скорее всего не аппаратные, 
  потому что точно также управляются флагом SSN.
```
...
Поддержка отладчиков PyOCD, OpenOCD, JLink, STLink
